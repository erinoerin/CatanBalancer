...
ax.set_position([0.2, 0.1, 0.7, 0.8])  # Adjust the board position to the right
ax.set_aspect("equal")
ax.axis([-7, 7, -7, 7])
plt.axis("off")
...
# Add a legend to explain port colors
legend_elements = [
    plt.Line2D([0], [0], color="green", lw=4, label="Sheep Port"),
    plt.Line2D([0], [0], color="#023020", lw=4, label="Wood Port"),
    plt.Line2D([0], [0], color="yellow", lw=4, label="Wheat Port"),
    plt.Line2D([0], [0], color="red", lw=4, label="Brick Port"),
    plt.Line2D([0], [0], color="gray", lw=4, label="Stone Port"),
    plt.Line2D([0], [0], color="black", lw=4, label="? Port"),
]
# Adjust the legend position to the left
ax.legend(
...
def generate_board():
    # Regenerate the board dynamically
    ...
    # Re-run the board generation logic
    global \
        numOfTileFails, \
        numOfNumberFails, \
        hasFailed, \
    ...
    outerCoord1.append(outerCoord1[0])
    ...
    xs, ys = zip(*outerCoord1)
    ax.plot(xs, ys, color="blue", alpha=oceanAlpha)

    outerCoord2 = np.transpose(outerCoord2)
    outerCoord2 = outerCoord2.tolist()
    xs, ys = zip(*outerCoord2)
    ax.plot(xs, ys, color="blue", alpha=oceanAlpha)

    outerCoord3 = np.transpose(outerCoord3)
    outerCoord3 = outerCoord3.tolist()
    xs, ys = zip(*outerCoord3)
    ax.plot(xs, ys, color="blue", alpha=oceanAlpha)

    outerCoord4 = np.transpose(outerCoord4)
    outerCoord4 = outerCoord4.tolist()
    xs, ys = zip(*outerCoord4)
    ax.plot(xs, ys, color="blue", alpha=oceanAlpha)

    outerCoord5 = np.transpose(outerCoord5)
    outerCoord5 = outerCoord5.tolist()
    xs, ys = zip(*outerCoord5)
    ax.plot(xs, ys, color="blue", alpha=oceanAlpha)

    outerCoord6 = np.transpose(outerCoord6)
    outerCoord6 = outerCoord6.tolist()
    xs, ys = zip(*outerCoord6)
    ax.plot(xs, ys, color="blue", alpha=oceanAlpha)
